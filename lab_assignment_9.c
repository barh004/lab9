...
void insertRecord(struct HashType *hashTable, struct RecordType *record, int tableSize)
{
    ...
    if(hashTable[index].list == NULL){
         // set 'record' equal to the HashType pointer in the table at index
         hashTable[index].list = record;
    }
        // set 'record' equal to the HashType pointer in the table at index
    // else
    else
    {
        struct RecordType* tmp = hashTable[index].list;
        while(tmp->next != NULL)
        {
            tmp = tmp->next;
        }
...
void displayRecordsInHash(struct HashType *hashTable, int tableSize)
{
    ...
    for(int x = 0; x < tableSize; x++)
    {
        ...
        {
            printf("index %d:", x);
            struct RecordType* tmp = hashTable[x].list;
            while(tmp != NULL)
            {
                printf("->");
...
int main(void)
{
    ...
    struct RecordType* tmp = pRecords;
